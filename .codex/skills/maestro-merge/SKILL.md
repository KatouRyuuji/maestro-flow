...
|-----------|-----------|
| Next dep-ready session | step `analyze` for session (`maestro run prepare --platform codex analyze --session {next-dep-ready-slug}` + `maestro run create analyze --session {next-dep-ready-slug} --intent "{goal}" --arg "{goal}"`) |
</completion>
