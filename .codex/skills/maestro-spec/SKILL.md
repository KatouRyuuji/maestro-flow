...
<dispatch>
Run `maestro run skill --platform codex specs-add` with the full `$ARGUMENTS` passed through as the user's intent (the `add` keyword is implied). The step infers category + scope + content and appends the entry.
