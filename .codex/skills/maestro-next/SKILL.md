...
- **Standard** (single run): recommend a step → confirm → execute via `maestro run prepare --platform codex` + `maestro run start`
...
```bash
maestro run prepare --platform codex   # check if prepare command works
...
maestro run prepare --platform codex <step>
