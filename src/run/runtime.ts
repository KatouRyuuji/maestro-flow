...
function buildFinishChecklist(projectRoot: string, run: CommandRun, frontmatter: ReportFrontmatter): string[] {
  ...
  lines.push('Teammate/agent outputs worth reusing: put conclusions into report.md frontmatter decisions/constraints (seal stages them automatically) or stage explicitly with `maestro knowledge stage knowhow "<title>" "<content>" --run ' + run.run_id + '` — never let sub-agent findings vanish at process exit.');
  lines.push('Negative knowledge: failed tests, rejected reviews, or contradicted practices → stage a counterexample candidate (scope + failure reason + alternative) with `maestro knowledge stage knowhow "<title>" "<content>" --run ' + run.run_id + '` so future runs stop repeating them.');
