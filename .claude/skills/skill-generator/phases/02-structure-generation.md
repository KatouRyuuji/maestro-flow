...
${sessionMode === 'run' ? `> **Run lifecycle** (see \`~/.maestro/workflows/run-mode.md\`): consume an injected birth packet's exact locator, resolved \`task\`, and structured \`continuation\` directly. Otherwise use the receipt-chained self-start flow: negotiate capabilities, \`session open\`, fenced \`session chain insert --command ${config.skill_name} --arg "<task text>"\`, then fenced \`run next\`, with identical participant/actor values, distinct request IDs, and each preceding receipt's exact revision. Domain text is positional; \`--input\` accepts only sealed same-Session Artifact IDs. Write formal artifacts under \`{run_dir}/outputs/\`. After \`maestro run check {run_id}\`, a self-started coordinator uses the canonical fenced completion from run-mode.md; a dispatched executor returns to its orchestrator.\n\n` : ''}\`\`\`javascript
