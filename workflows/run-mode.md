...
- Explicit `maestro load` / wiki loads are recorded as `consumed` on the unique active Run. Record stronger relations by stable ID with `maestro knowledge stage <target> "<title>" "<content>" --run {run_id} --signal cited|validated|contradicted --signal-ids <knowledge-ids>`.
