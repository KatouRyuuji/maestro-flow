...
- Stage reusable recipes, pitfalls, or other explicit candidates before completion with `maestro knowledge stage spec|knowhow "<title>" "<content>" --run {run_id} [--category <category>]`. For multiline content use `--content-file <path|->`. Routine Run completion MUST NOT call `maestro spec add` or `maestro knowhow add` directly.
...
5. Read the completion receipt. If it contains candidate IDs or reconciliation warnings, surface `maestro knowledge review {session_id}` as the review action; do not rewrite the same facts directly into spec/knowhow.
