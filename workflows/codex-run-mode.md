...
- Finish with `maestro run check {run_id}` and `maestro session done {run_id}`. When every gate is clean, `run check` emits a `finish` checklist (handoff, knowledge record, conflict marking, verdict, workflow-declared norms) — work through it before completing. Sealed artifacts are immutable. `run complete` remains the machine-compatible spelling.
