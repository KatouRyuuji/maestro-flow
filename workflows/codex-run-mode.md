...
- Finish with `maestro run check {run_id}` and `maestro session done {run_id} --session {session_id}`. When every gate is clean, `run check` emits a `finish` checklist (handoff, knowledge relation/candidate staging, conflict marking, verdict, workflow-declared norms) — work through it before completing. Read the returned knowledge candidate receipt and apply the Review Presentation Protocol: run `review --json` yourself, present each candidate needing disposition (content summary, evidence, matches, recommended disposition + rationale), collect the user's decisions, then execute `--resolve`; never hand the user the raw review command as the whole task. Do not directly write the same facts to project spec/knowhow. Sealed artifacts are immutable. `run complete` remains the machine-compatible spelling.
