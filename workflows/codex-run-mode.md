...
- Never edit `.workflow/state.json` artifact arrays or Session protocol JSON. Resolve inputs from the `upstream` map returned by `maestro session start` / `maestro run create`.
...
- Finish with `maestro run check {run_id}` and `maestro session done {run_id} --session {session_id}`. When every gate is clean, `run check` emits a `finish` checklist (handoff, knowledge relation/candidate staging, conflict marking, verdict, workflow-declared norms) — work through it before completing. Read the returned seal receipt and apply the Review Presentation Protocol: present each candidate needing disposition (content summary, evidence, matches, recommended disposition + rationale), collect the user's decisions, then execute the resolution — the happy path is `maestro knowledge promote {session_id} --resolve <candidate-id> --as <choice> [--target <knowledge-id>] --reason "<reason>"` (freshness fence + resolve + promote in one call); `review --resolve` remains the compatibility fallback for missing/stale receipt repair (`--refresh`), batch triage, and re-presentation. Never hand the user the raw command as the whole task. Do not directly write the same facts to project spec/knowhow. Sealed artifacts are immutable. `run complete` remains the machine-compatible spelling.
