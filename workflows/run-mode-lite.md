...
2. Before completion, stage reusable recipes/pitfalls with `maestro knowledge stage knowhow "<title>" "<content>" --run <run_id>`. Search/injection is exposure only; use `--signal cited|validated|contradicted --signal-ids <knowledge-ids>` on the same stage command for explicit relations.
