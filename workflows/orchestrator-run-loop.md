...
2. For new intent, classify and validate a chain definition. Each chain step declares `command/args/stage/goal_ref/decision_ref`. Prevalidate names with `maestro skills --steps --json --platform {target_platform}` where platform is `claude|codex|agent|agy|pi|grok`.
