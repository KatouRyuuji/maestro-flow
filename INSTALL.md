...
本仓库是 maestro-flow 的 **Grok Build** 适配层：先装官方 `maestro-flow@0.5.86`，再叠本项目的按文件补丁，使 Grok 成为一等宿主与 delegate 后端。
...
1. 已经（或先）全局安装官方包：`npm install -g maestro-flow@0.5.86`
2. 再在本仓库跑安装脚本，把本项目补丁叠到那个官方目录

官方版本必须与本仓库一致（当前 `0.5.86`）。不一致时脚本失败并提示手装匹配版本。源码比编译产物新时，一键脚本会先 `npm run build` 再覆盖。
...
官方包已经是 `0.5.86`、补丁已经叠上、只缺 `.grok/rules/maestro.md` 时，也可以直接再跑一次。脚本会再覆盖一遍（幂等），然后跑 `maestro install` 写资产。
...
官方包未装、或版本不等于本仓库（当前 `0.5.86`）时脚本失败并提示手动安装。
...
| 官方 Maestro | 必须已安装且版本等于本仓库（当前 `0.5.86`） | 未安装或版本不一致时脚本失败并提示手动安装 |
