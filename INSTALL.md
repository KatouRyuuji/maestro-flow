...
本仓库是独立迭代的 maestro-flow **fork**（版本号 `0.5.86-grok.N`）：先全局安装本仓构建的 tgz，再叠本项目的按文件补丁，使 Grok 成为一等宿主与 delegate 后端。
...
1. 已经（或先）全局安装本仓 fork 构建：`npm run build && npm pack && npm install -g ./maestro-flow-*.tgz`
2. 再在本仓库跑安装脚本，把本项目补丁叠到那个全局目录

全局包版本必须与本仓库一致（当前 `0.5.86-grok.N`，以 `package.json` 为准）。不一致时脚本失败并提示手装匹配版本。源码比编译产物新时，一键脚本会先 `npm run build` 再覆盖。
...
全局包已是本仓当前版本、补丁已经叠上、只缺 `.grok/rules/maestro.md` 时，也可以直接再跑一次。脚本会再覆盖一遍（幂等），然后跑 `maestro install` 写资产。
...
按提示确认即可。脚本会：检查全局 `maestro-flow` 版本 → 叠 Grok 按文件覆盖 → 写全局 MCP / 宿主资产 → 给调用方当前目录（或 `--path`）写 `.grok/`。

全局包未装、或版本不等于本仓库（以 `package.json` 为准）时脚本失败并提示手动安装。

常用标志：`--dry-run` 只检查不写盘；`-y` 跳过确认；`--rebuild` 强制重编译；`--path <dir>` 覆盖项目资产目录。

重装或升级全局包之后覆盖会被冲掉，再跑一次本脚本即可。
...
| 全局 Maestro | 必须已安装本仓 fork 构建且版本等于本仓库 `package.json` | 未安装或版本不一致时脚本失败并提示手动安装 |
...
2. **检查全局版本** — 全局 `maestro-flow` 是否等于本仓库版本；全局目录里有没有 Grok 覆盖
...
覆盖脚本只替换清单里的已编译文件（Grok adapter / 安装落点 / 本项目的 v3 hook 与知识可见性补丁 / hooks 遍历 http webhook 兼容补丁），**不整树替换** `dist/` / Dashboard，也**不改**全局包的 `node_modules` 依赖。全局目录会留下 `.maestro-grok-overlay.json` 方便核对叠了哪些文件。全局版本不一致时直接失败。
...
`apply-to-official` 只做按文件覆盖。一键脚本会跑 `maestro install`，但**不会**自动安装或降级全局包。
...
5. **重装或升级全局 `maestro-flow` 会冲掉覆盖**。再跑 `.\install.ps1` 即可。
...
9. **http webhook hook 兼容**：覆盖清单含 `dist/src/commands/hooks.js`。上游 hooks 遍历对 `h.command` 直接调 `includes`，遇到 `type:"http"`（只有 `url`、没有 `command`）的第三方 webhook hook 会崩溃并卡在安装「hooks」步；补丁改为 `(h.command ?? '')` 守卫，http hook 不匹配 maestro 标记，既不崩溃也不误删。

---

## 备选：直接使用本仓库 CLI

不经全局 tgz 安装、直接用本仓库源码时，在本仓库根构建后：
...
`npm link` 把本仓库链到全局 `maestro`。tgz 全局安装与 link 不要混用。

---

## 卸载覆盖

覆盖没有单独回滚命令。恢复为干净的全局包（重装本仓 fork 构建）：
