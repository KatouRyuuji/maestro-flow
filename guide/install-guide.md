...
```bash
# 1. 安装全局 CLI（本仓 fork 构建，版本见 package.json）
...
本仓库适配层推荐交互安装（先全局装本仓 fork 构建，再覆盖 + Grok 资产 / MCP）：
...
五步：检查依赖 → 检查全局版本 → 检查 Grok CLI → 模拟安装 → 确认后才写盘。每步会打印现状和安装方法。项目 `.grok/` 写到调用方当前目录，可用 `--path` 覆盖。全局包版本与本仓不一致时脚本失败并提示手装匹配版本。补丁已叠、只缺 `.grok/rules/maestro.md` 时，再跑一次同一脚本即可。
